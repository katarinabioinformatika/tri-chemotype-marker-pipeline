...
"""Scan a ProgressiveMauve TRI-core alignment for chemotype-separating amplicons."""
...
DEFAULT_OUTPUT = (
    ...
    / "diagnostic_amplicon_scan_mauve"
)
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scan a ProgressiveMauve TRI-core aligned FASTA for chemotype-separating amplicons."
    )
    parser.add_argument(
        "--alignment",
        type=Path,
...
def load_alignment(path: Path) -> list[SeqRecord]:
    if not path.exists():
        sys.exit(f"ERROR: ProgressiveMauve aligned FASTA not found: {path}")
...
def write_summary(
    ...
    lines = [
        ...
        "================================================",
        "",
        f"ProgressiveMauve aligned FASTA: {alignment}",
