...
ALIGNMENT_FILE = (
    ...
    / "mauve"
    / "core_TRI_cluster"
    / "Mauve_Alignment_extraction.fasta"
)
...
ALIGNMENT_METADATA_FILE = (
    OUT_DIR
    / "core_TRI_mauve_alignment_metadata.csv"
)
...
def assign_analysis_role(chemotype):
    """Assign each genome a role for later diagnostic-marker analysis."""
...
def extract_genome_id(fasta_header):
    """Extract the genome accession from one Mauve FASTA header."""
...
def main():
    """Check structural consistency of the core TRI-cluster Mauve alignment.

    This does not assess whether ProgressiveMauve produced the optimal
    biological alignment.
    """

    if not ALIGNMENT_FILE.exists():
        sys.exit(
            f"ERROR: Mauve alignment FASTA not found:\n{ALIGNMENT_FILE}"
        )
    ...
    if not records:
        sys.exit(
            "ERROR: No FASTA records were found in the Mauve alignment."
        )

    print(
        "\nCore TRI-cluster ProgressiveMauve alignment sanity check"
    )
    ...
    if missing_from_metadata:
        ...
        print(
            "\nGenome IDs present in Mauve alignment "
    ...
    if missing_from_alignment:
        ...
        print(
            ...
            "but missing from Mauve alignment:"
        )
    ...
    print(
        "\nProgressiveMauve alignment sequence mapping"
    )
    ...
    if problems:
        ...
        print(
            "\nThe ProgressiveMauve alignment is structurally "
            "consistent and suitable for downstream comparison."
        )
